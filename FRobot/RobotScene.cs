...
class RobotScene : Scene3 {
   ...
   public RobotScene () {
      ...
      Root       = new GroupVN ([
         ...
         new AxesVN { Name = "Dropoff", GetOrigin = () => new Point3 (ViewModel.BX, ViewModel.BY, ViewModel.BZ + 100) },
   ...
   internal void LoadScript (string path) {
      ...
      ViewModel.ScriptText = File.Exists (path) ? File.ReadAllText (path) : "";
      try {
         var ic = System.Globalization.CultureInfo.InvariantCulture;
...
#region class AxesVN -------------------------------------------------------------------------------
class AxesVN : VNode {
   public AxesVN () { Streaming = true; }
   public required string       Name      { get; init; }
   public required Func<Point3> GetOrigin { private get; init; }

   public override void SetAttributes () { Lux.ZLevel = 70; }

   public override void Draw () {
      var org = GetOrigin ();
      DrawArrow (org, Vector3.XAxis, Color4.Red);
      DrawArrow (org, Vector3.YAxis, Color4.Green);
      DrawArrow (org, Vector3.ZAxis, Color4.Blue);
   }
