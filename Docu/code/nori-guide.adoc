= Practical Mental Model: Nori

Nori is the internal rendering and geometry library that FRobot is built
on. You interact with it through three assemblies:
...
Your job is to: 1. Set `Lux.UIScene` to the scene you want rendered. 2.
Build a tree of `VNode` objects and assign it to `scene.Root`. 3.
Implement `Draw()` and `SetAttributes()` on each VNode.
...
Never do heavy computation in `Draw` — precompute in `SetAttributes` or
in response to data changes.

== When does `Draw` get called?

* *`Streaming = true`* — every frame, unconditionally (use for animated
content like `TcpVN` and `InfoVN`).
* *Default* — only when the node is marked dirty (`Redraw()` is called,
or the scene refreshes).
* *`++[++RedrawOnZoom++]++` attribute* — also re-draws when the user
pans or zooms (needed for any geometry expressed in screen-space units).
...
Set drawing properties _before_ the draw call — they are not parameters:
...
FRobot no longer uses sub-scenes (the fold-preview was removed with the
drawing editor).
...
VNodes live as long as you hold a reference. Add them to a `GroupVN` to
make them visible; remove them to hide them:
...
The mechanism computes forward kinematics automatically when `JValue`
changes. You just read `m.Xfm` to get the result.
...
|`Matrix3` |4×4 homogeneous transform; `Matrix3.Translation(x,y,z)`,
`.Rotation(EAxis.X, rad)`

|`CoordSystem` |A position {plus} three axes; `CoordSystem.World` =
identity

|`Bound3` |Axis-aligned bounding box in 3-D

|`Color4(a,r,g,b)` |ARGB color as bytes; `Color4.Red`, `.Blue`,
`.Gray(n)`
...
The `"pix:"` prefix is a registered asset namespace. In dev mode it maps
to `F:/Wad/`; in release it maps to `FRobot.wad` (a zip file).
