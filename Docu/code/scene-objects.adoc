...
The obstacle is a box centred on its BX/BY/BZ origin. Local Z runs from
−100 mm (bottom face) to {plus}100 mm (top face).
...
|==
|Name |Position |Notes
|*Dropoff* |(BX, BY, BZ {plus} 100) |Centre of the obstacle’s top face.
Tracks the obstacle as it moves.
...
Drawn by `TcpVN` — not an `AxesVN` instance and has no `Name`. Shows the
current tool-centre-point position and orientation, updated every frame
from the IK/FK solver.
