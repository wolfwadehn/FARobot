...
Nori is the internal rendering and geometry library that FApp is built on.
...
FApp no longer uses sub-scenes (the fold-preview was removed with the drawing
editor).
...
`F:/Wad/`; in release it maps to `FApp.wad` (a zip file).
